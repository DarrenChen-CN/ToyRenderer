...
void RenderBuffer::setFrameBuffer(float* frameBuffer){
...
float* RenderBuffer::getFrameBuffer(){
    return frameBuffer;
}

void RenderBuffer::setPixel(int x, int y, float z, Vec3& color){
    ...
    frameBuffer[idx * 4 + 0] = (unsigned char)newColor(0);
    frameBuffer[idx * 4 + 1] = (unsigned char)newColor(1);
    frameBuffer[idx * 4 + 2] = (unsigned char)newColor(2);
}